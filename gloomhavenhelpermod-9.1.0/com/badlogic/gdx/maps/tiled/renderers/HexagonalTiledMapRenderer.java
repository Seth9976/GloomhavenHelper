...
public class HexagonalTiledMapRenderer extends BatchTiledMapRenderer {
   ...
   private void init(TiledMap map) {
      ...
      if (length != null) {
         ...
         length = (Integer)map.getProperties().get("tilewidth", Integer.class);
         if (length != null) {
         ...
         length = (Integer)map.getProperties().get("tileheight", Integer.class);
         if (length != null) {
            this.hexSideLength = 0.5F * length.intValue();
         } else {
            TiledMapTileLayer tmtl = (TiledMapTileLayer)map.getLayers().get(0);
      ...
      if (this.staggerAxisX) {
         ...
         for (int row = row2 - 1; row >= row1; row--) {
            for (int col = colA; col < col2; col += 2) {
               this.renderCell(
                  layer.getCell(col, row), tileWidthUpperCorner * col + layerOffsetX, layerTileHeight50 + layerTileHeight * row + layerOffsetY, color
               );
            }
