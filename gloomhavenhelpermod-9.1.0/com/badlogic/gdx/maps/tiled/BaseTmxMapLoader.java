...
public abstract class BaseTmxMapLoader extends AsynchronousAssetLoader {
   ...
   public Array getDependencies(String fileName, FileHandle tmxFile, BaseTmxMapLoader.Parameters parameter) {
   ...
   protected abstract Array getDependencyAssetDescriptors(FileHandle var1, TextureLoader.TextureParameter var2);
   ...
   protected TiledMap loadTiledMap(FileHandle tmxFile, BaseTmxMapLoader.Parameters parameter, ImageResolver imageResolver) {
      ...
      for (int j = this.root.getChildCount(); i < j; i++) {
   ...
   protected void loadLayerGroup(TiledMap map, MapLayers parentLayers, XmlReader.Element element, FileHandle tmxFile, ImageResolver imageResolver) {
      if (element.getName().equals("group")) {
         ...
         for (int j = element.getChildCount(); i < j; i++) {
   ...
   protected void loadTileLayer(TiledMap map, MapLayers parentLayers, XmlReader.Element element) {
      if (element.getName().equals("layer")) {
         ...
         int tileWidth = (Integer)map.getProperties().get("tilewidth", Integer.class);
         int tileHeight = (Integer)map.getProperties().get("tileheight", Integer.class);
   ...
   protected void loadTileSet(XmlReader.Element element, FileHandle tmxFile, ImageResolver imageResolver) {
      if (element.getName().equals("tileset")) {
         ...
         Array tileElements = element.getChildrenByName("tile");
         this.addStaticTiles(
            tmxFile,
            imageResolver,
            tileSet,
         ...
         Array animatedTiles = new Array();

         for (XmlReader.Element tileElement : tileElements) {
            int localtid = tileElement.getIntAttribute("id", 0);
            TiledMapTile tile = tileSet.getTile(firstgid + localtid);
            if (tile != null) {
               AnimatedTiledMapTile animatedTile = this.createAnimatedTile(tileSet, tile, tileElement, firstgid);
   ...
   protected abstract void addStaticTiles(
      FileHandle var1,
      ImageResolver var2,
      TiledMapTileSet var3,
      XmlReader.Element var4,
      Array var5,
      String var6,
      int var7,
      int var8,
