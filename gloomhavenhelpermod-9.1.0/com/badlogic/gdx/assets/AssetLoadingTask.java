...
class AssetLoadingTask implements AsyncTask {
   ...
   public void unload() {
      if (this.loader instanceof AsynchronousAssetLoader) {
         ((AsynchronousAssetLoader)this.loader)
            .unloadAsync(this.manager, this.assetDesc.fileName, this.resolve(this.loader, this.assetDesc), this.assetDesc.params);
      }
   }
   ...
   private void removeDuplicates(Array array) {
      boolean ordered = array.ordered;
      array.ordered = true;

      for (int i = 0; i < array.size; i++) {
         String fn = ((AssetDescriptor)array.get(i)).fileName;
         Class type = ((AssetDescriptor)array.get(i)).type;

         for (int j = array.size - 1; j > i; j--) {
            if (type == ((AssetDescriptor)array.get(j)).type && fn.equals(((AssetDescriptor)array.get(j)).fileName)) {
               array.removeIndex(j);
            }
         }
      }
