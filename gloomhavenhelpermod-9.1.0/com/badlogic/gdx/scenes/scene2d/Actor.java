...
public class Actor {
   ...
   public void act(float delta) {
      ...
      if (actions.size != 0) {
         ...
         try {
            ...
            throw new RuntimeException("Actor: " + context.substring(0, Math.min(context.length(), 128)), var7);
         }
      }
   }
   ...
   public boolean fire(Event event) {
      ...
      Array ascendants = (Array<Group>)Pools.obtain(Array.class);
      ...
      try {
         ...
         if (event.isStopped()) {
            ...
            if (!event.getBubbles()) {
               ...
               for (int n = ascendants.size; ix < n; ix++) {
                  ((Group)ascendantsArray[ix]).notify(event, false);
                  if (event.isStopped()) {
                     return event.isCancelled();
                  }
               }
   ...
   public boolean notify(Event event, boolean capture) {
      if (event.getTarget() == null) {
         ...
         DelayedRemovalArray listeners = capture ? this.captureListeners : this.listeners;
         if (listeners.size == 0) {
            return event.isCancelled();
         } else {
            ...
            try {
               ...
               for (int n = listeners.size; i < n; i++) {
                  if (((EventListener)listeners.get(i)).handle(event)) {
                     event.handle();
                  }
               }
               ...
               String context = this.toString();
               throw new RuntimeException("Actor: " + context.substring(0, Math.min(context.length(), 128)), var6);
            }
   ...
   @Null
   public Actor firstAscendant(Class type) {
      if (type == null) {
         throw new IllegalArgumentException("actor cannot be null.");
      } else {
         Actor actor = this;
   ...
   public boolean isTouchFocusTarget() {
      ...
      if (stage == null) {
         ...
         for (int n = stage.touchFocuses.size; i < n; i++) {
            if (((Stage.TouchFocus)stage.touchFocuses.get(i)).target == this) {
               return true;
            }
         }
   ...
   public boolean isTouchFocusListener() {
      ...
      if (stage == null) {
         ...
         for (int n = stage.touchFocuses.size; i < n; i++) {
            if (((Stage.TouchFocus)stage.touchFocuses.get(i)).listenerActor == this) {
               return true;
            }
         }
         ...
         if (parent == null) {
            ...
            if (children.size == 1) {
               ...
               if (children.get(index) == this) {
                  return false;
               } else if (!children.removeValue(this, true)) {
                  return false;
               } else {
   ...
   public Vector2 localToScreenCoordinates(Vector2 localCoords) {
      Stage stage = this.stage;
      return stage == null ? localCoords : stage.stageToScreenCoordinates(this.localToAscendantCoordinates(null, localCoords));
   }
   ...
   public Vector2 localToAscendantCoordinates(@Null Actor ascendant, Vector2 localCoords) {
      ...
      do {
         ...
         actor = actor.parent;
      } while (actor != ascendant && actor != null);
