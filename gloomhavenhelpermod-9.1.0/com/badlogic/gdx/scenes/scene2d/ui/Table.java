...
public class Table extends WidgetGroup {
   ...
   public Cell add(@Null Actor actor) {
      Cell cell = this.obtainCell();
      cell.actor = actor;
      ...
      if (cellCount > 0) {
         Cell lastCell = (Cell)this.cells.peek();
         if (!lastCell.endRow) {
   ...
   public Cell add(@Null CharSequence text) {
      if (this.skin == null) {
         throw new IllegalStateException("Table must have a skin set to use this method.");
      } else {
   ...
   public Cell add(@Null CharSequence text, String labelStyleName) {
      if (this.skin == null) {
         throw new IllegalStateException("Table must have a skin set to use this method.");
      } else {
         return this.add(new Label(text, (Label.LabelStyle)this.skin.get(labelStyleName, Label.LabelStyle.class)));
      }
   }

   public Cell add(@Null CharSequence text, String fontName, @Null Color color) {
      if (this.skin == null) {
         throw new IllegalStateException("Table must have a skin set to use this method.");
      } else {
   ...
   public Cell add(@Null CharSequence text, String fontName, String colorName) {
      if (this.skin == null) {
         throw new IllegalStateException("Table must have a skin set to use this method.");
      } else {
   ...
   public Cell stack(@Null Actor... actors) {
   ...
   public float getRowHeight(int rowIndex) {
      return this.rowHeight == null ? 0.0F : this.rowHeight[rowIndex];
   }
   ...
   public float getColumnWidth(int columnIndex) {
      return this.columnWidth == null ? 0.0F : this.columnWidth[columnIndex];
   }
   ...
   private void computeSize() {
      ...
      for (int i = 0; i < cellCount; i++) {
         ...
         if (colspanx != 1) {
            ...
            if (this.round) {
               minWidthx = (float)Math.ceil(minWidthx);
               prefWidthx = (float)Math.ceil(prefWidthx);
            }
   ...
   public void layout() {
         ...
         if (extra > 0.0F) {
            ...
            for (int ix = 0; ix < rows; ix++) {
               if (expandHeight[ix] != 0.0F) {
                  float amount = extra * expandHeight[ix] / totalExpand;
                  rowHeight[ix] += amount;
                  used += amount;
                  lastIndex = ix;
               }
      ...
      float x = padLeft;
      if ((align & 16) != 0) {
         x = padLeft + (layoutWidth - tableWidth);
      } else if ((align & 8) == 0) {
         x = padLeft + (layoutWidth - tableWidth) / 2.0F;
      }

      float y = padTop;
      if ((align & 4) != 0) {
         y = padTop + (layoutHeight - tableHeight);
      } else if ((align & 2) == 0) {
         y = padTop + (layoutHeight - tableHeight) / 2.0F;
      }
      ...
      for (int ixxx = 0; ixxx < cellCount; ixxx++) {
         ...
         if ((align & 2) != 0) {
            ...
            c.actorY = (rowHeight[c.row] - c.actorHeight + c.computedPadTop - c.computedPadBottom) / 2.0F;
         }
      ...
      Array childrenArray = this.getChildren();
      Actor[] children = (Actor[])childrenArray.items;
      int ixxx = 0;

      for (int n = childrenArray.size; ixxx < n; ixxx++) {
         Object child = children[ixxx];
   ...
   private void addDebugRect(float x, float y, float w, float h, Color color) {
      Table.DebugRect rect = (Table.DebugRect)Table.DebugRect.pool.obtain();
      rect.color = color;
      rect.set(x, y, w, h);
   ...
   private void drawDebugRects(ShapeRenderer shapes) {
      if (this.debugRects != null && this.getDebug()) {
         ...
         for (int n = this.debugRects.size; i < n; i++) {
            Table.DebugRect debugRect = (Table.DebugRect)this.debugRects.get(i);
   ...
   public static class DebugRect extends Rectangle {
      static Pool pool = Pools.get(Table.DebugRect.class);
      Color color;
   }
}
