...
public class TextField extends Widget implements Disableable {
   ...
   public TextField(@Null String text, TextField.TextFieldStyle style) {
      this.setStyle(style);
   ...
   protected void calculateOffsets() {
      ...
      if (this.hasSelection) {
         ...
         float minXx = Math.max(glyphPositions[n] - glyphPositions[this.visibleTextStart], -this.textOffset);
   ...
   protected Drawable getBackgroundDrawable() {
      if (this.disabled && this.style.disabledBackground != null) {
         ...
         return this.style.focusedBackground != null && this.hasKeyboardFocus() ? this.style.focusedBackground : this.style.background;
      }
   }
   ...
   void updateDisplayText() {
      ...
      if (this.layout.runs.size > 0) {
         GlyphLayout.GlyphRun run = (GlyphLayout.GlyphRun)this.layout.runs.first();
   ...
   public void next(boolean up) {
      ...
      if (stage != null) {
         ...
         Vector2 currentCoords = this.getParent().localToStageCoordinates(tmp2.set(this.getX(), this.getY()));
   ...
   @Null
   private TextField findNextTextField(Array actors, @Null TextField best, Vector2 bestCoords, Vector2 currentCoords, boolean up) {
      int i = 0;
               ...
               if (this.checkFocusTraversal(character)) {
                  ...
                  if (add || remove) {
                     ...
                     if (add && !remove) {
                        ...
                        if (!TextField.this.withinMaxLength(
                           TextField.this.text.length() - (TextField.this.hasSelection ? Math.abs(TextField.this.cursor - TextField.this.selectionStart) : 0)
                        )) {
                           return true;
                        }
