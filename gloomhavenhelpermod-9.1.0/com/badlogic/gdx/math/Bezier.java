...
public class Bezier implements Path {
   ...
   public static Vector linear(Vector out, float t, Vector p0, Vector p1, Vector tmp) {
      return out.set(p0).scl(1.0F - t).add(tmp.set(p1).scl(t));
   }

   public static Vector linear_derivative(Vector out, float t, Vector p0, Vector p1, Vector tmp) {
      return out.set(p1).sub(p0);
   }

   public static Vector quadratic(Vector out, float t, Vector p0, Vector p1, Vector p2, Vector tmp) {
      float dt = 1.0F - t;
      return out.set(p0).scl(dt * dt).add(tmp.set(p1).scl(2.0F * dt * t)).add(tmp.set(p2).scl(t * t));
   }

   public static Vector quadratic_derivative(Vector out, float t, Vector p0, Vector p1, Vector p2, Vector tmp) {
      float dt = 1.0F - t;
      return out.set(p1).sub(p0).scl(2.0F).scl(1.0F - t).add(tmp.set(p2).sub(p1).scl(t).scl(2.0F));
   }

   public static Vector cubic(Vector out, float t, Vector p0, Vector p1, Vector p2, Vector p3, Vector tmp) {
      float dt = 1.0F - t;
      float dt2 = dt * dt;
      float t2 = t * t;
   ...
   public static Vector cubic_derivative(Vector out, float t, Vector p0, Vector p1, Vector p2, Vector p3, Vector tmp) {
      float dt = 1.0F - t;
      float dt2 = dt * dt;
      float t2 = t * t;
   ...
   public Bezier(Array points, int offset, int length) {
      this.set(points, offset, length);
   }

   public Bezier set(Vector... points) {
      return this.set(points, 0, points.length);
   }

   public Bezier set(Vector[] points, int offset, int length) {
   ...
   public Bezier set(Array points, int offset, int length) {
      if (length >= 2 && length <= 4) {
         if (this.tmp == null) {
   ...
   public float approximate(Vector v) {
      Vector p1 = (T)((Vector)this.points.get(0));
      Vector p2 = (T)((Vector)this.points.get(this.points.size - 1));
