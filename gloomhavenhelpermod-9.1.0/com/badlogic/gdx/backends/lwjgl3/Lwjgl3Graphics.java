...
public class Lwjgl3Graphics extends AbstractGraphics implements Disposable {
   ...
   @Override
   public int getWidth() {
      return this.window.getConfig().hdpiMode == HdpiMode.Pixels ? this.backBufferWidth : this.logicalWidth;
   }

   @Override
   public int getHeight() {
      return this.window.getConfig().hdpiMode == HdpiMode.Pixels ? this.backBufferHeight : this.logicalHeight;
   }
   ...
   public float getPpcX() {
      Lwjgl3Graphics.Lwjgl3Monitor monitor = (Lwjgl3Graphics.Lwjgl3Monitor)this.getMonitor();
      ...
      return (float)mode.width / sizeX * 10.0F;
   }
   ...
   public float getPpcY() {
      ...
      Graphics.DisplayMode mode = this.getDisplayMode();
      return (float)mode.height / sizeY * 10.0F;
   }
   ...
   public boolean setFullscreenMode(Graphics.DisplayMode displayMode) {
      ...
      if (this.isFullscreen()) {
         Lwjgl3Graphics.Lwjgl3DisplayMode currentMode = (Lwjgl3Graphics.Lwjgl3DisplayMode)this.getDisplayMode();
   ...
   public boolean setWindowedMode(int width, int height) {
      ...
      if (!this.isFullscreen()) {
         if (width != this.logicalWidth || height != this.logicalHeight) {
            ...
            this.window
               .setPosition(this.tmpBuffer.get(0) + (this.tmpBuffer3.get(0) - width) / 2, this.tmpBuffer2.get(0) + (this.tmpBuffer4.get(0) - height) / 2);
         }
         ...
         if (width == this.windowWidthBeforeFullscreen && height == this.windowHeightBeforeFullscreen) {
            GLFW.glfwSetWindowMonitor(
               ...
               0L,
               this.windowPosXBeforeFullscreen,
               this.windowPosYBeforeFullscreen,
               width,
               height,
               this.displayModeBeforeFullscreen.refreshRate
            );
         } else {
            Lwjgl3Graphics.Lwjgl3Monitor monitor = (Lwjgl3Graphics.Lwjgl3Monitor)this.getMonitor();
            GLFW.glfwGetMonitorWorkarea(monitor.monitorHandle, this.tmpBuffer, this.tmpBuffer2, this.tmpBuffer3, this.tmpBuffer4);
            GLFW.glfwSetWindowMonitor(
               this.window.getWindowHandle(),
               0L,
               this.tmpBuffer.get(0) + (this.tmpBuffer3.get(0) - width) / 2,
               this.tmpBuffer2.get(0) + (this.tmpBuffer4.get(0) - height) / 2,
               width,
               height,
               this.displayModeBeforeFullscreen.refreshRate
            );
