...
public abstract class OpenALMusic implements Music {
   ...
   protected void setup(int channels, int sampleRate) {
      ...
      this.maxSecondsPerBuffer = 40960.0F / (2 * channels * sampleRate);
   }
   ...
   public void setPan(float pan, float volume) {
      ...
      if (!this.audio.noDevice) {
         if (this.sourceID != -1) {
            AL10.alSource3f(this.sourceID, 4100, MathUtils.cos((pan - 1.0F) * (float) (Math.PI / 2)), 0.0F, MathUtils.sin((pan + 1.0F) * (float) (Math.PI / 2)));
            AL10.alSourcef(this.sourceID, 4106, volume);
         }
      }
   }
      ...
      if (!this.audio.noDevice) {
         if (this.sourceID != -1) {
            ...
            while (this.renderedSeconds < position - this.maxSecondsPerBuffer && this.read(tempBytes) > 0) {
               this.renderedSeconds = this.renderedSeconds + this.maxSecondsPerBuffer;
            }
   ...
   private boolean fill(int bufferID) {
      ...
      float currentBufferSeconds = this.maxSecondsPerBuffer * length / 40960.0F;
