...
public class ParticleEmitter {
   ...
   private void activateParticle(int index) {
      ...
      if (this.yScaleValue.active) {
         ...
         particle.setScale(
            particle.xScale + particle.xScaleDiff * this.xScaleValue.getScale(0.0F), particle.yScale + particle.yScaleDiff * this.yScaleValue.getScale(0.0F)
         );
      } else {
   ...
   private boolean updateParticle(ParticleEmitter.Particle particle, float delta, int deltaMillis) {
      ...
      if (life <= 0) {
         ...
         if ((updateFlags & 128) != 0) {
            int frame = Math.min((int)(percent * this.sprites.size), this.sprites.size - 1);
            if (particle.frame != frame) {
               Sprite sprite = (Sprite)this.sprites.get(frame);
         ...
         for (int n = this.particles.length; i < n; i++) {
            ...
            switch (this.spriteMode) {
               ...
               case animated:
                  ...
                  sprite = (Sprite)sprites.get(particle.frame);
                  break;
               case random:
   ...
   public void preAllocateParticles() {
      if (this.sprites.isEmpty()) {
         ...
         for (int index = 0; index < this.particles.length; index++) {
            ParticleEmitter.Particle particle = this.particles[index];
            if (particle == null) {
               this.particles[index] = particle = this.newParticle((Sprite)this.sprites.first());
               particle.flip(this.flipX, this.flipY);
            }
         }
      }
   ...
   public void setImagePaths(Array imagePaths) {
   ...
   public void scaleSize(float scaleX, float scaleY) {
      if (scaleX != 1.0F || scaleY != 1.0F) {
         for (ParticleEmitter.RangedNumericValue value : this.getXSizeValues()) {
            value.scale(scaleX);
         }

         for (ParticleEmitter.RangedNumericValue value : this.getYSizeValues()) {
            value.scale(scaleY);
         }
      }
   }
   ...
   public void scaleMotion(float scale) {
      if (scale != 1.0F) {
         for (ParticleEmitter.RangedNumericValue value : this.getMotionValues()) {
            value.scale(scale);
         }
      }
   }
   ...
   public void matchXSize(ParticleEmitter template) {
      ParticleEmitter.RangedNumericValue[] values = this.getXSizeValues();
      ParticleEmitter.RangedNumericValue[] templateValues = template.getXSizeValues();
   ...
   public void matchYSize(ParticleEmitter template) {
      ParticleEmitter.RangedNumericValue[] values = this.getYSizeValues();
      ParticleEmitter.RangedNumericValue[] templateValues = template.getYSizeValues();
   ...
   public void matchMotion(ParticleEmitter template) {
      ParticleEmitter.RangedNumericValue[] values = this.getMotionValues();
      ParticleEmitter.RangedNumericValue[] templateValues = template.getMotionValues();
   ...
   public void load(BufferedReader reader) throws IOException {
      try {
         ...
         if (line.startsWith("spriteMode")) {
            this.spriteMode = ParticleEmitter.SpriteMode.valueOf(readString(line));
            line = reader.readLine();
         }

         Array imagePaths = new Array();
         ...
         if (this.name == null) {
            ...
            throw new RuntimeException("Error parsing emitter: " + this.name, var4);
         }
      }
   }
   ...
   public static class IndependentScaledNumericValue extends ParticleEmitter.ScaledNumericValue {
      boolean independent;
      ...
      public void set(ParticleEmitter.RangedNumericValue value) {
         if (value instanceof ParticleEmitter.IndependentScaledNumericValue) {
            this.set((ParticleEmitter.IndependentScaledNumericValue)value);
         } else {
            super.set(value);
         }
      }
      ...
      public void set(ParticleEmitter.IndependentScaledNumericValue value) {
         super.set((ParticleEmitter.ScaledNumericValue)value);
      ...
      public void load(ParticleEmitter.IndependentScaledNumericValue value) {
         super.load(value);
         this.independent = value.independent;
      }
   }
   ...
   public static class ScaledNumericValue extends ParticleEmitter.RangedNumericValue {
      ...
      @Override
      public void set(ParticleEmitter.RangedNumericValue value) {
         if (value instanceof ParticleEmitter.ScaledNumericValue) {
            this.set((ParticleEmitter.ScaledNumericValue)value);
         } else {
            super.set(value);
         }
      }
