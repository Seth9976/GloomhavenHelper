...
public class BaseAnimationController {
   private final Pool transformPool = new Pool() {
   ...
   static final int getFirstKeyframeIndexAtTime(Array arr, float time) {
      int lastIndex = arr.size - 1;
      if (lastIndex > 0 && !(time < ((NodeKeyframe)arr.get(0)).keytime) && !(time > ((NodeKeyframe)arr.get(lastIndex)).keytime)) {
         int minIndex = 0;
   ...
   private static final Vector3 getTranslationAtTime(NodeAnimation nodeAnim, float time, Vector3 out) {
      if (nodeAnim.translation == null) {
         ...
         if (++index < nodeAnim.translation.size) {
            NodeKeyframe secondKeyframe = (NodeKeyframe<Vector3>)nodeAnim.translation.get(index);
   ...
   private static final Quaternion getRotationAtTime(NodeAnimation nodeAnim, float time, Quaternion out) {
      if (nodeAnim.rotation == null) {
         ...
         if (++index < nodeAnim.rotation.size) {
            NodeKeyframe secondKeyframe = (NodeKeyframe<Quaternion>)nodeAnim.rotation.get(index);
            float t = (time - firstKeyframe.keytime) / (secondKeyframe.keytime - firstKeyframe.keytime);
   ...
   private static final Vector3 getScalingAtTime(NodeAnimation nodeAnim, float time, Vector3 out) {
      if (nodeAnim.scaling == null) {
         ...
         if (++index < nodeAnim.scaling.size) {
            NodeKeyframe secondKeyframe = (NodeKeyframe<Vector3>)nodeAnim.scaling.get(index);
            float t = (time - firstKeyframe.keytime) / (secondKeyframe.keytime - firstKeyframe.keytime);
            out.lerp((Vector3)secondKeyframe.value, t);
         }
   ...
   private static final void applyNodeAnimationBlending(NodeAnimation nodeAnim, ObjectMap out, Pool pool, float alpha, float time) {
      Node node = nodeAnim.node;
      node.isAnimated = true;
      BaseAnimationController.Transform transform = getNodeAnimationTransform(nodeAnim, time);
      BaseAnimationController.Transform t = (BaseAnimationController.Transform)out.get(node, null);
      if (t != null) {
         if (alpha > 0.999999F) {
            t.set(transform);
         } else {
         ...
         out.put(node, ((BaseAnimationController.Transform)pool.obtain()).set(node.translation, node.rotation, node.scale).lerp(transform, alpha));
      }
   }

   protected static void applyAnimation(ObjectMap out, Pool pool, float alpha, Animation animation, float time) {
      if (out == null) {
         ...
         for (ObjectMap.Entry e : out.entries()) {
            if (!((Node)e.key).isAnimated) {
               ...
               ((BaseAnimationController.Transform)e.value).lerp(((Node)e.key).translation, ((Node)e.key).rotation, ((Node)e.key).scale, alpha);
            }
         }
      }
   }
