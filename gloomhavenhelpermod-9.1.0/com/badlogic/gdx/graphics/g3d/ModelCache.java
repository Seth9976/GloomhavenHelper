...
public class ModelCache implements Disposable, RenderableProvider {
   ...
   private FlushablePool renderablesPool = new FlushablePool() {
      protected Renderable newObject() {
         return new Renderable();
      }
   };
   private FlushablePool meshPartPool = new FlushablePool() {
   ...
   private Renderable obtainRenderable(Material material, int primitiveType) {
      Renderable result = (Renderable)this.renderablesPool.obtain();
      result.bones = null;
   ...
   public void end() {
      if (!this.building) {
         ...
         if (this.items.size != 0) {
            ...
            Renderable first = (Renderable)this.items.get(0);
            ...
            MeshPart part = this.meshBuilder.part("", primitiveType, (MeshPart)this.meshPartPool.obtain());
            this.renderables.add(this.obtainRenderable(material, primitiveType));
            int i = 0;

            for (int n = this.items.size; i < n; i++) {
               Renderable renderable = (Renderable)this.items.get(i);
               ...
               if (!samePart) {
                  if (!sameMesh) {
                     ...
                     while (offset < this.renderables.size) {
                        ((Renderable)this.renderables.get(offset++)).meshPart.mesh = mesh;
                     }
                  ...
                  MeshPart newPart = this.meshBuilder.part("", pt, (MeshPart)this.meshPartPool.obtain());
                  Renderable previous = (Renderable)this.renderables.get(this.renderables.size - 1);
   ...
   public void add(RenderableProvider renderableProvider) {
      ...
      for (int n = this.tmp.size; i < n; i++) {
         this.add((Renderable)this.tmp.get(i));
      }

      this.tmp.clear();
   }

   public void add(Iterable renderableProviders) {
   ...
   public static class SimpleMeshPool implements ModelCache.MeshPool {
      ...
      public Mesh obtain(VertexAttributes vertexAttributes, int vertexCount, int indexCount) {
         ...
         for (int n = this.freeMeshes.size; i < n; i++) {
            Mesh mesh = (Mesh)this.freeMeshes.get(i);
