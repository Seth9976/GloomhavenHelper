...
public class DefaultShader extends BaseShader {
   ...
   public DefaultShader(Renderable renderable, DefaultShader.Config config, ShaderProgram shaderProgram) {
      ...
      } else if (renderable.bones != null && renderable.bones.length > config.numBones) {
      ...
      if (this.pointLightsLoc >= 0) {
         for (int i = 0; i < this.pointLights.length; i++) {
            if (points != null && i < points.size) {
               if (this.lightsSet && this.pointLights[i].equals((PointLight)points.get(i))) {
                  continue;
               }
      ...
      if (this.spotLightsLoc >= 0) {
         for (int i = 0; i < this.spotLights.length; i++) {
            if (spots != null && i < spots.size) {
               if (this.lightsSet && this.spotLights[i].equals((SpotLight)spots.get(i))) {
                  continue;
               }
         ...
         public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
            if (renderable.environment == null) {
               ...
               if (combinedAttributes.has(DirectionalLightsAttribute.Type)) {
                  Array lights = ((DirectionalLightsAttribute)combinedAttributes.get(DirectionalLightsAttribute.Type)).lights;
               ...
               if (combinedAttributes.has(PointLightsAttribute.Type)) {
                  Array lights = ((PointLightsAttribute)combinedAttributes.get(PointLightsAttribute.Type)).lights;

                  for (int i = this.pointLightsOffset; i < lights.size; i++) {
                     this.cacheAmbientCubemap
                        .add(((PointLight)lights.get(i)).color, ((PointLight)lights.get(i)).position, tmpV1, ((PointLight)lights.get(i)).intensity);
                  }
               }
