...
public class CameraGroupStrategy implements GroupStrategy, Disposable {
   ...
   public CameraGroupStrategy(Camera camera, Comparator sorter) {
      this.camera = camera;
   ...
   public void beforeGroup(int group, Array contents) {
      if (group == 1) {
         ...
         for (int n = contents.size; i < n; i++) {
            ...
            Array materialGroup = (Array<Decal>)this.materialGroups.get(decal.material);
            if (materialGroup == null) {
               materialGroup = (Array<Decal>)this.arrayPool.obtain();
               materialGroup.clear();
