...
public class ParallelArray {
   ...
   public ParallelArray.Channel addChannel(ParallelArray.ChannelDescriptor channelDescriptor) {
      return this.addChannel(channelDescriptor, null);
   }

   public ParallelArray.Channel addChannel(ParallelArray.ChannelDescriptor channelDescriptor, ParallelArray.ChannelInitializer initializer) {
      ParallelArray.Channel channel = (T)this.getChannel(channelDescriptor);
      if (channel == null) {
         channel = (T)this.allocateChannel(channelDescriptor);
         if (initializer != null) {
   ...
   private ParallelArray.Channel allocateChannel(ParallelArray.ChannelDescriptor channelDescriptor) {
      if (channelDescriptor.type == float.class) {
         return new ParallelArray.FloatChannel(channelDescriptor.id, channelDescriptor.count, this.capacity);
      } else {
         return (ParallelArray.Channel)(channelDescriptor.type == int.class
   ...
   private int findIndex(int id) {
      for (int i = 0; i < this.arrays.size; i++) {
         ParallelArray.Channel array = ((ParallelArray.Channel[])this.arrays.items)[i];
         if (array.id == id) {
            return i;
         }
      }
   ...
   public ParallelArray.Channel getChannel(ParallelArray.ChannelDescriptor descriptor) {
   ...
   public interface ChannelInitializer {
      void init(ParallelArray.Channel var1);
   }
   ...
   public class ObjectChannel extends ParallelArray.Channel {
      Class componentType;
      public Object[] data;
      ...
      @Override
      public void setCapacity(int requiredCapacity) {
         Object[] newData = (T[])((Object[])ArrayReflection.newInstance(this.componentType, this.strideSize * requiredCapacity));
         System.arraycopy(this.data, 0, newData, 0, Math.min(this.data.length, newData.length));
         super.data = this.data = newData;
      }
   }
