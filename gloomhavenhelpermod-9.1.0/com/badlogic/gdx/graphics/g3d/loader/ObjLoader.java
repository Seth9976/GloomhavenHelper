...
public class ObjLoader extends ModelLoader {
   ...
   protected ModelData loadModelData(FileHandle file, boolean flipV) {
      ...
      for (int i = 0; i < this.groups.size; i++) {
         if (((ObjLoader.Group)this.groups.get(i)).numFaces < 1) {
      ...
      if (this.groups.size < 1) {
         ...
         for (int g = 0; g < numGroups; g++) {
            ObjLoader.Group group = (ObjLoader.Group)this.groups.get(g);
            ...
            while (ix < numElements) {
               int vertIndex = (Integer)faces.get(ix++) * 3;
               finalVerts[vi++] = this.verts.get(vertIndex++);
               finalVerts[vi++] = this.verts.get(vertIndex++);
               finalVerts[vi++] = this.verts.get(vertIndex);
               if (hasNorms) {
                  int normIndex = (Integer)faces.get(ix++) * 3;
               ...
               if (hasUVs) {
                  int uvIndex = (Integer)faces.get(ix++) * 2;
