...
public class FileHandle {
   ...
   public ByteBuffer map(MapMode mode) {
      if (this.type == Files.FileType.Classpath) {
         ...
         Object var5;
         try {
            raf = new RandomAccessFile(this.file, mode == MapMode.READ_ONLY ? "r" : "rw");
            FileChannel fileChannel = raf.getChannel();
            ByteBuffer map = fileChannel.map(mode, 0L, this.file.length());
            map.order(ByteOrder.nativeOrder());
            var5 = map;
         } catch (Exception var9) {
            throw new GdxRuntimeException("Error memory mapping file: " + this + " (" + this.type + ")", var9);
         } finally {
            StreamUtils.closeQuietly(raf);
         }

         return (ByteBuffer)var5;
      }
   }
