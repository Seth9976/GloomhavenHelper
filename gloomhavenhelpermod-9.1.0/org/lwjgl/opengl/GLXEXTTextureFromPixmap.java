...
public class GLXEXTTextureFromPixmap {
   ...
   public static void glXBindTexImageEXT(
      @NativeType("Display *") long display, @NativeType("GLXDrawable") long drawable, int buffer, @Nullable @NativeType("int const *") IntBuffer attrib_list
   ) {
      if (Checks.CHECKS) {
         Checks.checkNTSafe(attrib_list);
      }

      nglXBindTexImageEXT(display, drawable, buffer, MemoryUtil.memAddressSafe(attrib_list));
   }
   ...
   public static void glXBindTexImageEXT(
      @NativeType("Display *") long display, @NativeType("GLXDrawable") long drawable, int buffer, @Nullable @NativeType("int const *") int[] attrib_list
   ) {
      long __functionAddress = GL.getCapabilitiesGLXClient().glXBindTexImageEXT;
      if (Checks.CHECKS) {
         Checks.check(__functionAddress);
