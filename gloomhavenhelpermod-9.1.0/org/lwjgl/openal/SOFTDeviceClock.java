...
public class SOFTDeviceClock {
   ...
   @NativeType("ALCvoid")
   public static void alcGetInteger64vSOFT(
      @NativeType("ALCdevice *") long device, @NativeType("ALCenum") int pname, @NativeType("ALCint64SOFT *") LongBuffer values
   ) {
      nalcGetInteger64vSOFT(device, pname, values.remaining(), MemoryUtil.memAddress(values));
   }
   ...
   public static long alcGetInteger64vSOFT(@NativeType("ALCdevice *") long device, @NativeType("ALCenum") int pname) {
      ...
      try {
         ...
         nalcGetInteger64vSOFT(device, pname, 1, MemoryUtil.memAddress(values));
         var6 = values.get(0);
      } finally {
   ...
   @NativeType("ALCvoid")
   public static void alcGetInteger64vSOFT(
      @NativeType("ALCdevice *") long device, @NativeType("ALCenum") int pname, @NativeType("ALCint64SOFT *") long[] values
   ) {
      long __functionAddress = ALC.getICD().alcGetInteger64vSOFT;
      if (Checks.CHECKS) {
         Checks.check(__functionAddress);
